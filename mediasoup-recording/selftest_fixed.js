// Fixed self-test based on the working example approach
// Usage: node selftest_fixed.js
...
(async () => {
  try {
    ...
    // Use 127.0.0.1 instead of 0.0.0.0 to match the relay destination
    const recorderPort = await getPort();
    const probePort = rtpPort;

    // Use 127.0.0.1 to ensure proper binding
    const recorderAddr = '127.0.0.1';
...
`;
    ...
    const outPath = path.join(__dirname, `call_selftest_fixed.wav`);

    // Create FFmpeg instance using our wrapper (similar to how we do it in recording.js)
    const streamInfo = {
      audio: {
        remoteRtpPort: recorderPort,
    ...
    const filename = 'call_selftest_fixed';
    console.log('Creating recording process with streamInfo:', JSON.stringify(streamInfo, null, 2));
    
    const recorder = new FFmpeg(streamInfo, filename);
    console.log('FFmpeg recorder created');

    // Give recorder a moment to bind its UDP socket and be ready to receive
    ...
    relay.on('error', (err) => { 
      console.warn('relay error', err && err.message ? err.message : err); 
      relay.close(); 
    });
    ...
    await new Promise((resolve, reject) => {
      relay.bind(probePort, bindAddr, () => { 
        console.log('Relay bound on probePort', probePort, 'bindAddr', bindAddr); 
        resolve(); 
      });
    });
    ...
    const MAX_VERBOSE_RELAY = 5; // Always show first 5 packets
    ...
    relay.on('message', (msg, rinfo) => {
      ...
      relay.send(msg, 0, msg.length, recorderPort, '127.0.0.1', (err, bytes) => {
        if (err) {
          console.warn('[relay] send error', err && err.message ? err.message : err, { from: rinfo, to: { port: recorderPort, addr: '127.0.0.1' } });
        } else {
          relayForwarded++;
          if (relayForwarded <= MAX_VERBOSE_RELAY) console.log(`[relay] forwarded ${bytes} bytes to 127.0.0.1:${recorderPort} rtp=${isRtp}`);
        }
      });
    });
    ...
    const senderArgs = ['-re', '-f', 'lavfi', '-i', 'sine=frequency=440:sample_rate=48000', '-ac', '1', '-c:a', 'libopus', '-loglevel', 'info', '-f', 'rtp', `rtp://127.0.0.1:${probePort}?payload_type=${payloadType}`];
    ...
    sender.stderr.on('data', d => {
      const s = d.toString();
      if (/Error|failed|RTP:|Warning|could not/i.test(s)) {
    ...
    try { 
      ...
      console.log('Error stopping sender:', e.message);
    }
    ...
    if (fs.existsSync(outPath)) {
      ...
      console.log('Selftest recorded file:', outPath, 'size:', stats.size, 'bytes');
      console.log('SUCCESS: Recording created successfully!');
    } else {
      console.error('Selftest failed: output file not found at', outPath);
      // Check if directory exists
      const dirExists = fs.existsSync(path.dirname(outPath));
